...
def generate_simulated_photos_real(G,mu,imgsize,pixelsize,C,Ts,sumfactor,N_frames,R_minpixel,R_maxpixel):
    ...
    if R_pixel < (R_maxpixel+1):
        ...
        tracks = np.sum(densitytracks,axis=0) # density tracks
    ...
    return observ_stacked, velocitymap, vessel_gt , veltracks, tracks 
...
def create_real_data(N,G,mu,imgsize,pixelsize,C,Ts,sumfactor,N_frames,R_minpixel,R_maxpixel):
    ...
    for i in tqdm(range(N)):
        obs,vel,gt , veltra, tra = generate_simulated_photos_real(G,mu,imgsize,pixelsize,C,Ts,sumfactor,N_frames,R_minpixel,R_maxpixel)
    ...
    combined = obs_summed[:, None,:,:] 
    combined = np.append(combined, tracks, axis = 1)
    combined = np.append(combined, vessel_GT, axis = 1)
    combined = np.append(combined, veltracks, axis = 1)
    combined = np.append(combined, velocitymap, axis = 1)
