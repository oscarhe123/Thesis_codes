...
class PseudoClassificationLoss(nn.Module):
    def __init__(self, thresholds,device,lossweight=1e-3):
        super(PseudoClassificationLoss, self).__init__()
        self.thresholds = thresholds
        self.lossweight= lossweight
        self.device = device

    def forward(self, regression_output, regression_target):
        # Discretize regression predictions into classes
        pseudo_labels = torch.bucketize(regression_output, self.thresholds).type(torch.float) -1
        #print(pseudo_labels)
        pseudo_labels.to(self.device)
        
        

        # Convert regression targets to pseudo-class labels
        pseudo_targets = torch.bucketize(regression_target, self.thresholds).type(torch.float) -1
        pseudo_targets.to(self.device)
        #print(pseudo_targets)

        # Apply Cross-Entropy Loss
        classification_loss = nn.BCELoss()
        loss = classification_loss(pseudo_labels, pseudo_targets) 
...
class ConvOverlapBLSTM(nn.Module):
    def __init__(self, input_size, input_channels, hidden_channels, num_layers, device):
        ...
        self.device = device
        self.forward_net = ConvLSTM(input_size, input_channels, hidden_channels, kernel_size=(5, 5), num_layers=num_layers,bias = False, device=device)
        self.reverse_net = ConvLSTM(input_size, input_channels, hidden_channels, kernel_size=(5, 5), num_layers=num_layers,bias = False, device=device)
        self.conv_net = nn.Sequential(nn.Conv2d(2 * self.hidden_channels, 128, kernel_size=5,bias = False, padding=2),nn.BatchNorm2d(128),
                                      nn.ReLU(),
                                      nn.Conv2d(128, 256, kernel_size=5,bias = False, padding=2),nn.BatchNorm2d(256),
                                      nn.ReLU(),
                                      nn.Conv2d(256, 64, kernel_size=5,bias = False, padding=2),nn.BatchNorm2d(64),
                                      nn.ReLU(),
                                      nn.Conv2d(64, 1, kernel_size=5,bias = False, padding=2))
